...
$pos = array_search('-h', $t_args) ?? array_search('--help', $t_args);

if ($pos) {
    fprintf(STDERR, "Info:\tyakpro-po version = %s%s", Config::YAKPRO_PO_VERSION, PHP_EOL . PHP_EOL);
    $lang = '';
    if (($x = getenv('LANG')) !== false) {
        $s = strtolower($x);
    } $x = explode('_', $x);
    $x = $x[0];
    if (file_exists("$yakpro_po_dirname/locale/$x/README.md")) {
        $help = file_get_contents("$yakpro_po_dirname/locale/$x/README.md");
    } elseif (file_exists("$yakpro_po_dirname/README.md")) {
        $help = file_get_contents("$yakpro_po_dirname/README.md");
    } else {
...
$pos = array_search('-o', $t_args) ?? array_search('--output-file', $t_args);
...
if (($x = getenv('YAKPRO_PO_CONFIG_FILENAME')) !== false) {
    $config_file_namepart = $x;
                                                            ...
                                                            $t_where    = array();
if ($argument_config_filename != '') {
    $t_where[]  = $argument_config_filename;                        // --config-file argument
}
if (($x = getenv('YAKPRO_PO_CONFIG_FILE'))     !== false) {
    $t_where[]  = $x;                                               // YAKPRO_PO_CONFIG_FILE
}
if (($x = getenv('YAKPRO_PO_CONFIG_DIRECTORY')) !== false) {
    $t_where[]  = "$x/$config_file_namepart";                       // YAKPRO_PO_CONFIG_DIRECTORY
}
...
if (($x = getenv('HOME')) !== false) {
    $t_where[]  = "$x/$config_file_namepart";                       // HOME
}
if ($x                  !== false) {
    $t_where[]  = "$x/config/$config_file_namepart";                // HOME/config
}
...
if (!$conf->silent) {
    fprintf(STDERR, "Info:\tyakpro-po version = %s%s", Config::YAKPRO_PO_VERSION, PHP_EOL);
}
...
switch ($process_mode) {
    case 'file':
        ...
        if (!$conf->silent) {
            fprintf(STDERR, "Info:\ttarget_file\t\t= [%s]%s", (!empty($target_file)) ? $target_file : 'stdout', PHP_EOL);
        }
        break;
    case 'directory':
        if (!$conf->silent) {
